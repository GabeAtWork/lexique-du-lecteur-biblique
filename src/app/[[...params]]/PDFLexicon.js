...
export default function PDFLexicon({ frequency, data }) {
  ...
  const dataByChap = data.reduce((acc, word) => {
    if (acc[word.chapter]) {
  ...
  const writePageHeaderFooter = (pageNb, book, chapter, verse) => {
    doc
      .setFont('Times', 'italic')
      .setFontSize(8)
  ...
  const writeWord = (word, y, colNb, writeVerseNb) => {
    const columnOffset = colNb * (columnWidth + columnGutter)
    ...
    // lex
    const isHebrew = word.strong[0] === 'H'
  ...
  for (const currentChapterNb of Object.keys(dataByChap)) {
    ...
    while (dataToWrite.length > 0) {
      ...
      if (dataToWriteLines >= 2 * columnAvailableLines) {
        ...
        // first column
        for (const word of columnData[0]) {
          writeWord(word, currentY, 0, currentVerseNb !== word.verse)
          currentVerseNb = word.verse
          currentY += lineHeightToMm(word.gloss.length) + padding[0] // pt to mm
        }

        const bottomOfColumn = currentY

        // second column
        currentY = topColumnY

        for (const word of columnData[1]) {
          writeWord(word, currentY, 1, currentVerseNb !== word.verse)
          currentVerseNb = word.verse
